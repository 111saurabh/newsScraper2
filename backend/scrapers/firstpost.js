...
/**
 * Firstpost News Scraper
 * Scrapes articles from Firstpost news website
 */
class FirstpostScraper {
  constructor() {
    this.baseUrl = "https://www.firstpost.com";
    this.categories = {
      tech: "/tech",
      politics: "/politics",
      sports: "/sports",
  ...
  async getArticlesByCategory(category, limit = 10) {
    try {
      ...
      $(".big-thumb, .list-item").each((i, element) => {
        if (i >= limit) return false;

        const title = $(element).find("h3, h2").text().trim();
        const summary = $(element).find(".excerpt, .description").text().trim();
        ...
        const publishedAt =
          $(element).find(".post-info time").attr("datetime") || "";
      ...
      console.error(`Error scraping Firstpost ${category}:`, error);
      return [];
    }
  }
  ...
  async getArticleContent(url) {
    try {
      ...
      const author = $(".author-name").text().trim() || "Firstpost Staff";
      const keywords = [];
...
module.exports = FirstpostScraper;
